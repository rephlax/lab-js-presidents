...
function getNames(presidentsArr) {
  ...
  let newArr = presidentsArr.map(president => president.name);
...
function  countYearsInOffice(presidentsArr) {
  ...
  return presidentsArr.reduce((total, current) => {

    if (current.leftOffice !== null){
      const yearsInOffice = current.leftOffice - current.tookOffice;
      total += yearsInOffice;
    }
...
function sortPresidentsByBirthYear(presidentsArr) {
  ...
  return presidentsArr.sort(compareFunc);
}
...
function countRepublicanPresidents(presidentsArr) {
  ...
  return presidentsArr.reduce((total, current) => {
    ...
    return current.party === "Republican" ? total + 1 : total;
...
function sortPresidentsByName(presidentsArr) {
  ...
  return presidentsArr.sort(compareFunc2); 
}
